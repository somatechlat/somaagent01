...
- **Runtime Engine Policy:** Manifests SHALL declare `runtime_engine` (orchestrator|local|external); local runs SHALL be limited (e.g., ≤5s CPU, ≤50MB heap, no unapproved egress, no background jobs); orchestrator SHALL be required for multi-step, bulk, exports, syncs, or HITL flows; enforcement SHALL occur at install and runtime with fallback to orchestrator when limits exceed.
