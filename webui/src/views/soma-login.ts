...
@customElement('soma-login')
export class SomaLogin extends LitElement {
    static styles = css`
        :host {
            display: block;
            min-height: 100vh;
    ...
    private async _handleLogin(e: Event) {
        ...
        try {
            const authProvider = document.querySelector('soma-auth-provider') as any;
            if (authProvider) {
