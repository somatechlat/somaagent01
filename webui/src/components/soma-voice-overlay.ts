...
import './soma-button.js';

@customElement('soma-voice-overlay')
export class SomaVoiceOverlay extends LitElement {
    static styles = css`
        :host {
            display: contents;
        }
        ...
        .overlay {
            ...
            z-index: var(--soma-z-overlay, 300);
            display: flex;
        ...
        .visualizer-core {
            ...
            inset: 60px;
            border-radius: 50%;
            background: linear-gradient(135deg, var(--soma-accent, #94a3b8), var(--soma-info, #3b82f6));
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 32px;
        ...
        .hint {
            position: absolute;
            bottom: var(--soma-spacing-xl, 32px);
...
declare global {
    interface HTMLElementTagNameMap {
        'soma-voice-overlay': SomaVoiceOverlay;
    }
}
