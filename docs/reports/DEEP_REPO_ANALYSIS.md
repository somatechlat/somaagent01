...
- `admin/core/chat_orchestrator.py` and `services/common/chat/*`
- `admin/core/context/builder.py`, `services/common/simple_governor.py`
...
- `config/settings_registry.py`, `services/gateway/settings.py`, `admin/core/api/settings_v2.py`
- `webui/src/views/*`
...
- `admin/core/api/settings_v2.py` reads legacy names like `POSTGRES_HOST`, `REDIS_URL`, `KAFKA_BROKERS`, `KEYCLOAK_URL`.
...
- `admin/conversations/api.py` implements overlapping conversation endpoints and calls `admin/core/chat_orchestrator.py` (but this router is not mounted in `admin/api.py`).
- `admin/core/chat_orchestrator.py` + `services/common/chat/*` implement a split chat pipeline with SimpleGovernor, ContextBuilder, ModelRouter, and memory bridge.
...
- `admin/core/context/builder.py` provides a concrete context builder with SomaBrain + circuit breaker integration.
...
- `admin/common/session_manager.py` manages agent sessions tied to conversations, but uses the same class name `SessionManager`.
...
- JWT validation is in `admin/common/auth.py` (JOSE).
- Policy enforcement exists in `services/common/authorization.py` (OPA).
...
1) Placeholder logic still in core path
...
Pick one canonical chat pipeline: `admin/core/chat_orchestrator.py` (V3 pipeline). Remove or hard-disable other entry points, then update routers to use the canonical path only.
