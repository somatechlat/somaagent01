...
setattr(settings, "SOMA_API_TOKEN", os.environ.get("SOMA_API_TOKEN"))
# Patch namespace and other things SFM might need
setattr(settings, "SOMA_MEMORY_NAMESPACE", "test_ns")
setattr(settings, "SOMA_LOG_LEVEL", "INFO")

from services.common.degradation_monitor import DegradationMonitor
from somafractalmemory.api.schemas import MemoryStoreRequest
...
async def run_degradation_verification():
    ...
    try:
        ...
        if not data.get("vector_store"):
            ...
            print_result("Vector Store Health", False, "Reported as healthy despite Milvus being down!")
    ...
    try:
        ...
        if resp.status_code == 200:
             print_result("Graceful Failure", True, "Returned 200 OK! System gracefully degraded (Saved to SQL, skipped Milvus)")
        elif resp.status_code == 503 or resp.status_code == 424:
             print_result("Graceful Failure", False, "Returned error stats, system did not degrade gracefully.")
        else:
