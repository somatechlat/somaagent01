"""GatewayService – wraps the existing FastAPI gateway as a BaseSomaService.

The original gateway lives in ``services.gateway.main`` and is a fully‑featured
FastAPI application.  To satisfy the *Standardised Service Lifecycle* goal from
...
sub‑application mounted by the orchestrator’s FastAPI instance.  Nevertheless we
...
class GatewayService(BaseSomaService):
    """Wrap the existing ``services.gateway.main`` FastAPI app.
    ...
    def __init__(self) -> None:
        ...
        # The gateway is a FastAPI instance; we keep a reference for health.
    ...
    async def _stop(self) -> None:
        # No resources to clean up; the FastAPI shutdown event will be handled
