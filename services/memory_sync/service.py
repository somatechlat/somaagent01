"""Memory sync service based on BaseService for orchestrator integration.

This service inherits from BaseService to be managed by the orchestrator.
It wraps the existing memory sync worker functionality.
"""
...
class MemorySyncService(BaseService):
    """Memory sync service that inherits from BaseService for orchestrator integration."""

    service_name: str = "memory-sync"
    ...
    async def startup(self) -> None:
        """Initialize memory sync service and start the worker."""
        LOGGER.info(f"Starting {self.service_name} service")
        
        try:
            # Import the MemorySyncWorker from the original module
            from .main import MemorySyncWorker
            
            # Create the worker instance
            self.worker = MemorySyncWorker()
    ...
    async def shutdown(self) -> None:
        """Clean up memory sync service resources."""
    ...
    def register_routes(self, app: FastAPI) -> None:
        """Register health check endpoints for the memory sync service."""
        ...
        @app.get("/metrics")
        async def metrics():
            """Return basic metrics about the memory sync service."""
            return {
    ...
    def as_dict(self) -> Dict[str, Any]:
        """Return a serialisable representation of the memory sync service."""
        base_info = super().as_dict()
        base_info.update({
            "port": self.config.memory_sync_port,
            "worker_running": self.worker_task is not None and not self.worker_task.done(),
        })
        return base_info