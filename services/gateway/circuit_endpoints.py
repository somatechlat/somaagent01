...
async def get_component_circuit_status(component_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not circuit:
            raise HTTPException(status_code=404, detail=f"Circuit breaker for {component_name} not found")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get circuit status for {component_name}: {str(e)}")
...
async def reset_circuit_breaker(component_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        metrics_collector.track_error(
            error_type="circuit_breaker_reset",
            location=f"circuit_endpoints.{component_name}"
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to reset circuit breaker for {component_name}: {str(e)}")
...
async def force_open_circuit_breaker(component_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Track metrics
        metrics_collector.track_error(
            error_type="circuit_breaker_force_open",
            location=f"circuit_endpoints.{component_name}"
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to force open circuit breaker for {component_name}: {str(e)}")
...
async def get_circuit_config(component_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "failure_threshold": circuit.failure_threshold,
            "recovery_timeout": circuit.recovery_timeout,
            "expected_exception": str(circuit.expected_exception) if circuit.expected_exception else None,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get circuit config for {component_name}: {str(e)}")
...
async def update_circuit_config(
    ...
    try:
        ...
        # Track metrics
        metrics_collector.track_error(
            error_type="circuit_config_update",
            location=f"circuit_endpoints.{component_name}"
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update circuit config for {component_name}: {str(e)}")
...
async def get_circuit_history(component_name: str, limit: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get circuit history for {component_name}: {str(e)}")
...
async def get_circuit_service_health() -> Dict[str, Any]:
    ...
    try:
        ...
        open_circuits = sum(1 for circuit in all_circuits.values() if circuit.state == CircuitState.OPEN)
...
async def test_circuit_breaker(component_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to test circuit breaker for {component_name}: {str(e)}")