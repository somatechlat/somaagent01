...
async def record_memory_persistence_metric(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to record memory persistence metric: {str(e)}")


@router.post("/memory/wal-lag")
async def record_wal_lag_metric(
    lag_seconds: float,
    tenant: str = "default"
) -> Dict[str, Any]:
    """
...
async def record_policy_decision_metric(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to record policy decision metric: {str(e)}")
        ...
        if format == "prometheus":
            ...
            for collector in REGISTRY._collector_to_names.keys():
                try:
                    for metric in collector.collect():
                        for sample in metric.samples:
                            labels_str = ",".join([f"{k}={v}" for k, v in (sample.labels or {}).items()])
