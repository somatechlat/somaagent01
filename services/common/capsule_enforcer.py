...
class ResourceEnforcer:
    ...
    def check_wall_clock(self) -> EnforcementResult:
        ...
        return EnforcementResult(
            ...
            details={"elapsed": elapsed, "remaining": remaining, "limit": self.max_wall_clock_seconds}
        )
...
class HITLEnforcer:
    ...
    def check_action(
        ...
        if self.default_mode == "none":
            return EnforcementResult(
                action=EnforcementAction.ALLOW,
                reason="HITL mode is 'none' - no approval required"
            )
        ...
        if self.default_mode == "optional":
            ...
            if risk_score >= threshold:
                ...
                return EnforcementResult(
                    ...
                    details={"action_type": action_type, "risk_score": risk_score, "threshold": threshold}
                )
        ...
        return EnforcementResult(
            action=EnforcementAction.REQUIRE_HITL,
            reason=f"Unknown HITL mode: {self.default_mode}"
        )
