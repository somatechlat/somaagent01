...
class ManifestParser:
    ...
    RISK_FIELDS = {"tool_risk_profile", "risk_thresholds", "data_classification"}

    HITL_FIELDS = {"default_hitl_mode", "max_pending_hitl"}
    ...
    def validate(self, manifest: Dict[str, Any]) -> ManifestValidationResult:
        ...
        if "default_hitl_mode" in manifest:
            if manifest["default_hitl_mode"] not in self.VALID_HITL_MODES:
                errors.append(
                    f"Invalid default_hitl_mode: {manifest['default_hitl_mode']}. "
        ...
        if "tool_risk_profile" in manifest:
            if manifest["tool_risk_profile"] not in self.VALID_RISK_PROFILES:
                errors.append(
                    f"Invalid tool_risk_profile: {manifest['tool_risk_profile']}. "
        ...
        if "data_classification" in manifest:
            if manifest["data_classification"] not in self.VALID_DATA_CLASSIFICATIONS:
                errors.append(
                    f"Invalid data_classification: {manifest['data_classification']}. "
        ...
        if (
            manifest.get("tool_risk_profile") in {"high", "critical"}
            and manifest.get("default_hitl_mode") == "none"
        ):
            warnings.append("High-risk capsule with no HITL - consider requiring human approval")

        return ManifestValidationResult(valid=len(errors) == 0, errors=errors, warnings=warnings)
    ...
    def _validate_prod_restrictions(self, manifest: Dict[str, Any]) -> List[str]:
        ...
        if not manifest.get("opa_policy_packages") and not manifest.get("guardrail_profiles"):
            errors.append(
                "Capsules in PROD must have at least one policy package or guardrail profile"
            )
        ...
        if manifest.get("tool_risk_profile") in {"high", "critical"}:
            if manifest.get("default_hitl_mode") == "none":
                errors.append(
                    "High/critical risk capsules require HITL in PROD (default_hitl_mode != 'none')"
                )

        return errors

    def to_capsule_record(
        self, manifest: Dict[str, Any], tenant_id: str, generate_id: bool = True
    ) -> CapsuleRecord:
        ...
        if not result.valid:
            raise ManifestValidationError(
                f"Manifest validation failed: {result.errors}", errors=result.errors
            )
