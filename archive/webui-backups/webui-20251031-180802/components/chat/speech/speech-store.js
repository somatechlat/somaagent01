...
const model = {
  ...
  async _createRealtimeConnection() {
    // Establish a WebRTC connection to OpenAI Realtime via Gateway SDP proxy
    try {
      ...
      dc.onmessage = (ev) => this.handleRealtimeMessage(ev);
      dc.onopen = () => {
        // Flush any pending messages
        const pending = this.realtime.pendingMessages.splice(0);
        for (const msg of pending) {
          try { dc.send(msg); } catch (_) {}
        }
      };

      // Attach microphone as a sending track (send to OpenAI) and also receive remote audio
      // Do not interfere with existing STT flow; this capture is scoped to realtime only.
      try {
        if (!this.realtime.micStream) {
          ...
          this.realtime.micStream = micStream;
        }
        // Add all audio tracks to the peer connection
        this.realtime.micStream.getAudioTracks().forEach((track) => {
          try { pc.addTrack(track, this.realtime.micStream); } catch (_) {}
        });
        // Ensure we also receive remote audio
        pc.addTransceiver("audio", { direction: "sendrecv" });
      } catch (e) {
        console.warn("Realtime mic capture unavailable:", e);
        // Still allow recv-only if capture fails
        try { pc.addTransceiver("audio", { direction: "recvonly" }); } catch(_){}
      }
      pc.ontrack = (ev) => this.handleRealtimeTrack(ev);

      // Create and set local offer
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);

      // Send offer to Gateway; it will forward to OpenAI and return SDP answer
      const resp = await fetchApi('/v1/speech/openai/realtime/offer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sdp: offer.sdp, model: this.speech_realtime_model || 'gpt-4o-realtime-preview' }),
      });

      if (resp.status === 404) {
        window.toastFrontendWarning?.('OpenAI credentials missing. Set the key in Settings → Keys.', 'Speech', 5);
        this.closeRealtimeConnection();
        throw new Error('missing_credentials');
      }
      if (!resp.ok) {
        const txt = await resp.text();
        window.toastFetchError?.('Realtime connection failed', new Error(txt || `HTTP ${resp.status}`));
        this.closeRealtimeConnection();
        throw new Error(`realtime_http_${resp.status}`);
      }
  ...
  closeRealtimeConnection() {
    const { peerConnection, dataChannel, remoteAudioEl, micStream } = this.realtime;
  ...
  async speakWithKokoro(text, waitForPrevious = false, terminator = null) {
    ...
    try {
      const resp = await fetchApi('/v1/speech/tts/kokoro', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (resp.status === 501) {
        window.toastFrontendWarning?.("Kokoro is not available on this server. Using browser TTS.", "Speech", 4);
        return this.speakWithBrowser(text, waitForPrevious, terminator);
      }
      if (resp.status === 503) {
        window.toastFrontendInfo?.("Kokoro is initializing. Please try again in a moment.", "Speech", 4);
        return; // let the loop attempt next chunk later
      }
      if (!resp.ok) {
        const msg = await resp.text();
        throw new Error(`Kokoro HTTP ${resp.status}: ${msg}`);
      }
      const data = await resp.json();
      const audio = data && data.audio;
      if (!audio) throw new Error("Kokoro response missing audio");
      await this.playAudio(audio);
    } catch (error) {
      console.error("Kokoro TTS error:", error);
      window.toastFetchError?.("Kokoro TTS error", error);
      return this.speakWithBrowser(text, waitForPrevious, terminator);
    }
  },
...
class MicrophoneInput {
  ...
  handleListeningState() {
    ...
    // When using OpenAI Realtime, ensure the peer connection is up so mic is streamed live
    try {
      if (store.speech_provider === 'openai_realtime') {
        // Fire and forget; errors are surfaced via toasts elsewhere
        store.ensureRealtimeConnection().catch(() => {});
      }
    } catch (_) {}
  }
  ...
  async process() {
    ...
    // reset chunks buffer for the next cycle
    this.audioChunks = [];

    let base64;
    try {
      base64 = await this.convertBlobToBase64Wav(audioBlob);
    } catch (err) {
      console.error("Failed to convert audio to base64:", err);
      window.toastFrontendError?.("Audio conversion error", "Speech");
      this.status = Status.LISTENING;
      return;
    }
    ...
    // Call backend STT
    try {
      const resp = await fetchApi('/v1/speech/transcribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          audio: base64,
          language: store.stt_language,
          model_size: store.stt_model_size,
        }),
      });

      if (resp.status === 501) {
        if (!this._sttWarned) {
          window.toastFrontendWarning?.("Speech-to-text is not available on this server.", "Speech", 5);
      ...
      if (!resp.ok) {
        const txt = await resp.text();
        throw new Error(`STT HTTP ${resp.status}: ${txt}`);
      }

      const data = await resp.json();
      const text = (data && typeof data.text === 'string') ? data.text : '';
      const filtered = this.filterResult(text || '');
      if (filtered) {
        await this.updateCallback(filtered, true);
      }
    } catch (err) {
      console.error("STT request failed:", err);
      window.toastFetchError?.("Speech-to-text failed", err);
    } finally {
      this.status = Status.LISTENING;
    }
  }
