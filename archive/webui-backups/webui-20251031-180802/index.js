...
let __lastProvisionalContext = null; // holds the most recent client-minted session id until server confirms
...
// Offline toast gating for SSE: only notify if offline persists beyond a grace window
let __sseOfflineSince = null;
let __sseOfflineToastTimer = null;
// Seen event ids to suppress duplicate renders across history preload and SSE
const _seenEventIds = new Set();
// Per-session turn counter to give thinking bubbles distinct, persistent ids
const _turnCounterBySession = {};
// Toggle chat input/send enabled state based on whether a chat is selected
function updateInputEnabled() {
  try {
    const hasContext = !!(context && String(context).trim() !== "");
    if (sendButton) {
      sendButton.disabled = !hasContext;
      sendButton.classList.toggle('disabled', !hasContext);
    }
    if (chatInput) {
      if (!chatInput.dataset.placeholder) chatInput.dataset.placeholder = chatInput.placeholder || 'Type your message here...';
      chatInput.disabled = !hasContext;
      chatInput.placeholder = hasContext ? chatInput.dataset.placeholder : 'Select a chat or click New Chat';
    }
  } catch (_) {}
}
// Track the last user event id per session to thread assistant replies
const _lastUserEventBySession = {};
// Context switch timestamp to guard against Delete/Select races
let __contextSwitchedAt = 0;
...
function renderGatewayEvent(sessionId, event) {
  try {
    ...
    const evId = event.event_id || event.id || null;
    ...
    // Strong duplicate suppression for non-stream events by event id (avoid history+SSE duplicates)
    if (!isStreaming && evId && kind !== 'uploads.progress') {
      if (_seenEventIds.has(evId)) return;
      _seenEventIds.add(evId);
    }

    if (role === "assistant") {
      const turnId = meta.reply_to_event_id || meta.user_event_id || meta.request_id || _lastUserEventBySession[sessionId] || null;
      // Dedicated Thinking… bubble (separate from the answer bubble); do not reuse streaming id
      if (kind === "assistant.thinking" || status === "thinking") {
        const kvps = meta && (meta.analysis ? { thoughts: meta.analysis } : { ...meta });
        if (!_turnCounterBySession[sessionId]) _turnCounterBySession[sessionId] = 0;
        _turnCounterBySession[sessionId] += 1;
        const thinkingId = evId || `assistant-thinking-${sessionId}-${_turnCounterBySession[sessionId]}`;
        setMessage(thinkingId, "agent", "Thinking…", "", false, kvps);
        return;
      }
      // Suppress streaming chunk renders entirely to avoid a duplicate answer bubble.
      // Keep a marker so the first non-streaming final can reuse/align if needed, but don't render.
      if (isStreaming) {
      ...
      if (_sseActiveAssistantBySession[sessionId]) {
        // On final, prefer server event_id; fall back to the tracked in-progress id
        stableId = evId || _sseActiveAssistantBySession[sessionId];
        delete _sseActiveAssistantBySession[sessionId];
      }
      // If this is a final assistant message, dedupe any prior assistant responses for the same user turn
      if (kind === 'assistant.final' && turnId) {
        try {
          const containers = Array.from(document.querySelectorAll('#chat-history .message-agent-response'));
          for (const el of containers) {
            if (el && el.parentElement) {
              const container = el.closest('.message-container');
              if (!container) continue;
              const cid = container.id && container.id.replace(/^message-/, '');
              if (container.dataset && container.dataset.turn === String(turnId) && cid && cid !== String(stableId)) {
                container.remove();
              }
            }
          }
      ...
      // Render confirmed user message using server event id to merge with optimistic DOM
      if (evId) stableId = evId;
      setMessage(stableId, "user", "User message", content, false, { ...meta });
      if (evId) _lastUserEventBySession[sessionId] = evId;
      return;
    } else if (role === "tool") {
    ...
    // (Thinking handled above in the assistant-role block; no second path here.)
    ...
    if (role === "tool" || kind === "tool.start" || kind === "tool.result") {
      ...
      // Suppress low-signal internal tools to keep the UI focused (helps strict Playwright assertions too)
      const SUPPRESS_TOOLS = new Set(["file_read"]);
      if (SUPPRESS_TOOLS.has(String(toolName).toLowerCase())) {
        return; // skip rendering this tool lifecycle entirely
      }
    ...
    const containerEl = setMessage(stableId, type, heading, content, isStreaming, kvps);
    try {
      if (type === 'response') {
        const turnId = meta.reply_to_event_id || meta.user_event_id || meta.request_id || _lastUserEventBySession[sessionId] || null;
        if (turnId && containerEl) {
          const mc = containerEl.querySelector('.message');
          (mc || containerEl).closest('.message-container').dataset.turn = String(turnId);
        }
      }
    } catch (_) {}
  } catch (e) {
...
function openGatewayStream(sessionId) {
  ...
  es.onerror = () => {
    ...
    // Start a quiet grace window; only surface a toast if offline persists beyond 8 seconds
    if (!__sseOfflineSince) {
      __sseOfflineSince = Date.now();
      if (__sseOfflineToastTimer) { try { clearTimeout(__sseOfflineToastTimer); } catch(_) {} }
      __sseOfflineToastTimer = setTimeout(() => {
        if (!__sseConnected) {
          try {
            notificationStore.frontendWarning(
              "Live updates temporarily offline – retrying…",
              "Connection",
              8,
              "sse-offline"
            );
  ...
  es.onopen = () => {
    ...
    __sseConnected = true;
    __sseOfflineSince = null;
    if (__sseOfflineToastTimer) {
      try { clearTimeout(__sseOfflineToastTimer); } catch(_) {}
      __sseOfflineToastTimer = null;
    }
  };
}

async function waitForSSEOpen(sessionId, timeoutMs = 1500) {
  const start = Date.now();
  // If already connected for this session, return
  if (_gatewaySSE && _gatewaySSESession === sessionId && __sseConnected) return true;
  // Poll briefly until connected
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize input enabled state on load
  try { updateInputEnabled(); } catch (_) {}
});
...
export async function sendMessage() {
  ...
  __sendInFlight = true;
  try {
    // Require an active chat context (gold UI parity)
    if (!context || String(context).trim() === "") {
      __sendInFlight = false;
      try { notificationStore.frontendWarning('Select a chat or click New Chat to start.', 'No chat selected', 4, 'no-chat-selected'); } catch (_) {}
      return;
    }
    ...
    if (message || hasAttachments) {
      ...
      // Ensure the SSE stream is open for the current context
      try { openGatewayStream(context); } catch (_) {}

      // Guard: if we very recently switched contexts (e.g., after Delete), wait a beat for SSE to open
      try {
        const switchedAgo = Date.now() - __contextSwitchedAt;
        if (switchedAgo < 250) {
          await waitForSSEOpen(context, 1500);
        }
      } catch (_) {}
      ...
      // Do not add an optimistic Thinking… placeholder here. Rely on the real
      // assistant.thinking SSE event so the user bubble appears first.
      ...
      if (!jsonResponse) {
        ...
        if (jsonResponse.accepted && jsonResponse.session_id) {
          ...
          // If the server minted a different session id than our provisional one, reconcile the list
          try {
            if (__provisionalContext === true && __lastProvisionalContext && __lastProvisionalContext !== jsonResponse.session_id) {
              replaceChatListId(__lastProvisionalContext, jsonResponse.session_id);
            }
          } catch (_) {}
          ...
          try {
            ...
            if (jsonResponse.event_id && jsonResponse.session_id) {
              _lastUserEventBySession[jsonResponse.session_id] = jsonResponse.event_id;
            }
          } catch (_) {}
...
async function loadSessionHistory(sessionId) {
  ...
  try {
    ...
    for (const entry of body.events) {
      ...
      if (!payload) continue;
      // Preserve the top-level event id and session mapping for dedupe
      if (!payload.event_id && (entry.event_id || entry.id)) payload.event_id = entry.event_id || entry.id;
      if (!payload.session_id && (entry.session_id || entry.context || sessionId)) payload.session_id = entry.session_id || sessionId;
...
globalThis.resetChat = async function (ctxid = null) {
  try {
    ...
    try {
      delete _sseActiveAssistantBySession[sid];
      delete _lastAssistantBySession[sid];
      delete _turnCounterBySession[sid];
      delete _lastUserEventBySession[sid];
      _uploadMessageByFilename.clear && _uploadMessageByFilename.clear();
    } catch (_) {}
    setContext(sid, { preserveHistory: false, force: true });
    resetCounter++;
...
globalThis.killChat = async function (id) {
  ...
  try {
    ...
    const deletingCurrent = (context === id);
    ...
    // If we deleted the currently selected chat, clear selection and disable input until user selects or creates a chat
    if (deletingCurrent) {
      try { closeGatewayStream(); } catch (_) {}
      context = "";
      try { chatHistory.innerHTML = ""; } catch(_) {}
      updateInputEnabled();
      // Clear selection in list
      try { if (chatsAD) chatsAD.selected = ""; } catch(_) {}
    }
...
export const newContext = function () {
  // Use a UUID v4-style id so the Gateway accepts it as a valid session id
  // This prevents the server from minting a different id and avoids "random" chats appearing.
  const newId = generateGUID();
  setContext(newId, { preserveHistory: false, force: true });
  try { window.__newContextCount++; } catch(_) {}
  // Show a local placeholder in the chats list immediately; merge with server list when available
  try {
    if (globalThis.Alpine && chatsSection) {
      const chatsAD = Alpine.$data(chatsSection);
      const exists = (Array.isArray(chatsAD?.contexts) ? chatsAD.contexts : []).some(c => c.id === newId);
      if (!exists) {
        const list = Array.isArray(chatsAD?.contexts) ? chatsAD.contexts.slice() : [];
        list.unshift({ id: newId, name: 'New chat', no: 1 });
        chatsAD.contexts = list;
        chatsAD.selected = newId;
      }
    }
    refreshChatsList();
  } catch (_) {}
...
export const setContext = function (id, options = {}) {
  if (id == context && !(options && options.force === true)) return;
  const preserve = options && options.preserveHistory === true;
  context = id;
  __contextSwitchedAt = Date.now();
  ...
  try {
    (async () => {
      ...
      try { if (id && String(id).trim() !== "") openGatewayStream(id); } catch (_) {}
    })();
  } catch (_) {}
  // Toggle input enabled state based on whether we have a context
  try { updateInputEnabled(); } catch(_) {}
};

function replaceChatListId(oldId, newId) {
  if (!oldId || !newId || oldId === newId) return;
  try {
    if (!(globalThis.Alpine && chatsSection)) return;
    const chatsAD = Alpine.$data(chatsSection);
    if (!chatsAD || !Array.isArray(chatsAD.contexts)) return;
    let changed = false;
    const next = [];
    for (const c of chatsAD.contexts) {
      if (c.id === oldId) {
        // If a row with newId already exists, drop the old; otherwise replace id
        if (chatsAD.contexts.some(x => x.id === newId)) {
          changed = true;
          continue; // drop the old placeholder
        } else {
          next.push({ ...c, id: newId });
          changed = true;
        }
      } else {
        next.push(c);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Dedupe notification bell icons in case a component mounts twice due to race conditions
  try {
    const ensureSingleBell = () => {
      const host = document.getElementById('time-date-container') || document;
      const bells = host.querySelectorAll('.notification-toggle');
      if (bells && bells.length > 1) {
        // keep the first, remove the rest
        for (let i = 1; i < bells.length; i++) {
          try { bells[i].remove(); } catch (_) {}
        }
      }
    };
    // Run now and shortly after x-components finish mounting
    ensureSingleBell();
    setTimeout(ensureSingleBell, 150);
    // Observe late mounts and enforce single instance
    const bellObserver = new MutationObserver(() => ensureSingleBell());
    const target = document.getElementById('time-date-container') || document.body;
    bellObserver.observe(target, { childList: true, subtree: true });
  } catch (_) {}
});
...
async function refreshChatsList() {
  try {
    ...
    let mapped = arr.map((s, idx) => {
      const sid = s.session_id || s.id || s.sessionId;
      const metaTitle = (s.metadata && (s.metadata.title || s.metadata.name)) || '';
      const title = s.subject || metaTitle || `Chat ${String(sid || '').slice(0, 8)}`;
      return { id: sid, name: title, no: idx + 1 };
    });
    // If current context is not yet on server, keep a placeholder so the list doesn’t appear empty
    if (context && !mapped.some(c => c.id === context)) {
      mapped = [{ id: context, name: 'New chat', no: 1 }, ...mapped];
    }
    if (globalThis.Alpine && chatsSection) {
      const chatsAD = Alpine.$data(chatsSection);
      if (chatsAD) {
...
function ensureDefaultChatSelected() {
  ...
  if (list.length === 0) {
    // Agent Zero UX: auto-create/select a new chat on first load
    try {
      newContext();
    } catch (_) {}
