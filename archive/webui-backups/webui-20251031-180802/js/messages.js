...
function drawKvpsIncremental(container, kvps, latex) {
  if (kvps) {
    ...
    kvpEntries.forEach(([key, value], index) => {
      ...
      // Special pretty rendering for 'thoughts' object
      if ((key === 'thoughts' || key === 'reasoning') && value && typeof value === 'object' && !Array.isArray(value)) {
        const wrap = document.createElement('div');
        wrap.classList.add('thoughts-wrap');
        const intent = value.intent ?? value.classification ?? undefined;
        const sentiment = value.sentiment ?? undefined;
        const tags = Array.isArray(value.tags) ? value.tags : (typeof value.tags === 'string' ? value.tags.split(/\s*,\s*/) : undefined);
        const rows = [];
        if (intent) rows.push(`<strong>Intent:</strong> ${escapeHTML(String(intent))}`);
        if (sentiment) rows.push(`<strong>Sentiment:</strong> ${escapeHTML(String(sentiment))}`);
        if (tags && tags.length) rows.push(`<strong>Tags:</strong> ${escapeHTML(tags.join(', '))}`);
        // Fallback to full JSON if no known fields are present
        if (rows.length === 0) {
          addValue(JSON.stringify(value, null, 2), tdiv);
        } else {
          wrap.innerHTML = rows.map(r => `<div class="thoughts-row">${r}</div>`).join('');
