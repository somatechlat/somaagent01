...
async def run_test():
    ...
    async with async_playwright() as p:
        ...
        def on_request_failed(req):
            try:
                failed_requests.append(
                    (req.url, req.failure.error_text if req.failure else "unknown error")
                )
            except Exception:
                pass
            ...
            if chat:
                ...
                try:
                    ...
                    async with page.expect_request(
                        lambda r: "/v1/session/message" in r.url and r.method == "POST",
                        timeout=15000,
                    ) as req_info:
                    ...
                    try:
                        async with page.expect_response(
                            lambda r: r.url == req.url and r.request.method == "POST", timeout=15000
                        ) as resp_info:
                            pass
                        ...
                        print(
                            "No response observed for /v1/session/message within 15s (request was sent)."
                        )
                except PlaywrightTimeoutError:
        ...
        if ai_msg:
            ...
            print(
                "No AI reply found in the DOM within 30s. Ensure workers are running and connected to Kafka/LLM."
            )
