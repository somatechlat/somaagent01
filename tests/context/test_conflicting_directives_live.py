...
async def test_conflicting_directives_favor_latest_instruction():
    ...
    try:
        ...
        correction = (
            "Actually cancel the staging deployment. Instead, prepare the production rollout checklist."
        )
        ...
        assert "cancel" in window_text, "latest cancellation directive not emphasized in context window"
        assert "production" in window_text or "prod" in window_text, (
        ...
        combined_extras = "\n".join(
            filter(
                None,
                [
                    json.dumps(extras["memories"], ensure_ascii=False) if extras["memories"] else "",
                    json.dumps(extras["solutions"], ensure_ascii=False) if extras["solutions"] else "",
                ],
            )
        ).lower()
        if combined_extras.strip():
            assert "cancel" in combined_extras or "production" in combined_extras, (
