...
class ExternalServiceConfig(BaseModel):
    ...
    # SomaClient Settings
    soma_circuit_breaker_threshold: int = Field(
        default=3, description="Circuit breaker failure threshold"
    )
    soma_circuit_breaker_cooldown: float = Field(
        default=15.0, description="Circuit breaker cooldown in seconds"
    )
    soma_max_retries: int = Field(default=2, description="Max retries for SomaBrain requests")
    soma_retry_base_ms: int = Field(default=150, description="Base retry backoff in ms")
...
class DecisionConfig(BaseModel):
    ...
    base_urgency: float = Field(default=0.5, description="Base urgency score")
    action_bias_multiplier: float = Field(
        default=0.4, description="Multiplier for action bias (dopamine)"
    )
    stability_multiplier: float = Field(
        default=0.2, description="Multiplier for stability score (serotonin)"
    )
    threshold_go: float = Field(default=0.8, description="Threshold for GO decision")
    threshold_nogo: float = Field(default=0.3, description="Threshold for NOGO decision")
...
class Config(BaseModel):
    ...
    decision: DecisionConfig = Field(
        default_factory=DecisionConfig, description="Decision Engine configuration"
    )
