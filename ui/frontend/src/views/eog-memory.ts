...
@customElement('eog-memory')
export class EogMemory extends LitElement {
    static styles = css`
        :host {
            display: block;
        ...
        .spinner {
            width: 32px;
            height: 32px;
            border: 3px solid var(--eog-border-color, rgba(255, 255, 255, 0.1));
        ...
        return html`
            ...
            <div class="search-bar">
                <eog-input
                    ...
                    @eog-input=${this._handleSearchInput}
                    @keyup=${this._handleSearchKeyup}
                ></eog-input>
                <eog-button @click=${this._performSearch}>Search</eog-button>
            </div>
    ...
    private _renderMemoryCard(item: SearchResult) {
        ...
        return html`
            <eog-card class="memory-card" clickable @eog-click=${() => this._viewMemory(memory)}>
    ...
    private _viewMemory(memory: Memory) {
        this.dispatchEvent(new CustomEvent('eog-view-memory', {
            detail: { memory },
            bubbles: true,
            composed: true,
        }));
