...
async def update_integration(request, provider: str, payload: IntegrationUpdate) -> IntegrationConfig:
    ...
    try:
        if provider == "lago":
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                success = response.status_code == 200
                message = "Keycloak realm accessible" if success else f"Error: {response.status_code}"
            ...
            sock.settimeout(5)
            result = sock.connect_ex((config.get("endpoint", "localhost"), config.get("smtp_port", 587)))
            sock.close()
            success = result == 0
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                success = response.status_code in [200, 403]  # 403 = valid but no access without auth
