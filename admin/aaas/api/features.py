"""
Features API Router
Manage AAAS features and feature flags.
...
# =============================================================================
# AAAS FEATURES (for Tier Builder)
# =============================================================================
@router.get("", response=list[FeatureOut])
def list_features(request):
    """Get all available AAAS features for tier builder."""
    features = AaasFeature.objects.filter(is_active=True).order_by("sort_order")
    return [
        FeatureOut(
            id=str(f.id),
            code=f.code,
...
def get_feature(request, feature_id: str):
    ...
    try:
        f = AaasFeature.objects.get(id=feature_id)
        return FeatureOut(
            id=str(f.id),
            code=f.code,
            name=f.name,
...
def assign_feature_to_tier(
    ...
    try:
        feature = AaasFeature.objects.get(code=feature_code)
    except AaasFeature.DoesNotExist:
        from django.http import Http404
