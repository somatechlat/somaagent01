# SOMA Stack: Unified AAAS (Direct Mode) Deployment Guide
> **Version**: 1.0.0 (Jan 2026)
> **Compliance**: ISO/IEC 29148:2018
> **Architecture**: Unified AAAS / Direct Integration

## 1. Overview
This guide routes the deployment of the **Unified AAAS (Direct Mode)**, where `somaAgent01`, `somabrain`, and `somafractalmemory` run within the **same Python process** (or shared runtime space) to achieve sub-millisecond memory operations.

### Key Architecture Differences
| Feature | Standard AAAS (HTTP) | Unified AAAS (Direct) |
| :--- | :--- | :--- |
...
| **Config** | `SOMA_AAAS_MODE=true` | `SOMA_AAAS_MODE=direct` |
...
Create or update `infra/aaas_deployment/.env` with the specific Direct Mode flags.

```bash
# infra/aaas_deployment/.env

# --- CORE ARCHITECTURE MODES ---
SOMA_AAAS_MODE=direct                 # <--- ENABLES DIRECT MEMORY INTEGRATION
...
The `start_aaas.sh` script automatically handles:
...
```bash
docker compose logs -f somastack_aaas | grep "Direct Mode"
```
*Expected Output:*
