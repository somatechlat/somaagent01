...
@shared_task(
    bind=True,
    base=SafeTask,
    name="python.tasks.conversation_tasks.delegate",
    max_retries=3,
...
@shared_task(
    bind=True,
    base=SafeTask,
    name="python.tasks.conversation_tasks.store_interaction",
    max_retries=2,
    autoretry_for=(Exception,),
    retry_backoff=True,
    retry_jitter=True,
...
def store_interaction(self, session_id: str, interaction: dict[str, Any]) -> dict[str, Any]:
    ...
    validate_payload(
        STORE_INTERACTION_PAYLOAD_SCHEMA, {"session_id": session_id, "interaction": interaction}
    )
    _enforce_policy(
        "store_interaction",
        interaction.get("tenant_id", "unknown"),
        "store_interaction",
        {
...
@shared_task(
    bind=True,
    base=SafeTask,
    name="python.tasks.conversation_tasks.feedback_loop",
    max_retries=2,
    autoretry_for=(Exception,),
    retry_backoff=True,
    retry_jitter=True,
...
def feedback_loop(self, session_id: str, feedback: dict[str, Any]) -> dict[str, Any]:
    ...
    _enforce_policy(
        "feedback_loop",
        feedback.get("tenant_id", "unknown"),
        "feedback_loop",
