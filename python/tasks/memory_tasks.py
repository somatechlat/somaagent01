...
@shared_task(
    bind=True,
    base=SafeTask,
    name="python.tasks.memory_tasks.rebuild_index",
    max_retries=1,
...
def rebuild_index(self, tenant_id: str) -> dict[str, Any]:
    ...
    _enforce_policy(
        "rebuild_index", tenant_id, "rebuild_index", {"name": "rebuild_index", "tenant": tenant_id}
    )
...
@shared_task(
    bind=True,
    base=SafeTask,
    name="python.tasks.memory_tasks.evaluate_policy",
    max_retries=2,
    autoretry_for=(Exception,),
    retry_backoff=True,
    retry_jitter=True,
