...
class BaseTask(Task):
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        try:
            redis_client.hset(
                f"task:{task_id}",
                mapping={"status": "failed", "error": str(exc), "failed_at": time.time()},
            )
        except RedisError:
...
def a2a_chat_task(
    ...
    """
    # Validate arguments against the declared schema (VIBE security)
    validate_payload(
    ...
    if not is_client_available():
        ...
        increment_counter(
            fast_a2a_errors_total,
            {"agent_url": agent_url, "error_type": "client_unavailable", "method": "celery_task"},
        )
    ...
    try:
        ...
        redis_client.setex(f"fast_a2a_session:{agent_url}", 86400, session_id)  # 24 hour TTL

    except RedisError as e:
        increment_counter(
            fast_a2a_errors_total,
            {"agent_url": agent_url, "error_type": "redis_error", "method": "celery_task"},
        )
    ...
    try:
        with fast_a2a_latency_seconds.labels(agent_url=agent_url, method="celery_task").time():
            ...
            asyncio.set_event_loop(store_loop)
            try:
                store_loop.run_until_complete(
        ...
        increment_counter(
            fast_a2a_errors_total,
            {"agent_url": agent_url, "error_type": type(e).__name__, "method": "celery_task"},
        )
        ...
        try:
            redis_client.hset(
                f"task:{task_id}",
                mapping={"status": "failed", "error": str(e), "failed_at": time.time()},
            )
        except RedisError:
            pass
...
async def _store_conversation_in_redis(
    session_id: str, message: str, reply: str, attachments: List[str]
) -> None:
    """
    ...
    try:
        ...
        redis_client.rpush(f"conversation:{session_id}", json.dumps(user_message))

        # Store assistant reply
        assistant_message = {"role": "assistant", "content": reply, "timestamp": time.time()}

        redis_client.rpush(f"conversation:{session_id}", json.dumps(assistant_message))
