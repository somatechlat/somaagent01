...
@shared_task(
    bind=True,
    base=SafeTask,
    name="python.tasks.maintenance_tasks.publish_metrics",
    max_retries=1,
...
@shared_task(
    bind=True,
    base=SafeTask,
    name="python.tasks.maintenance_tasks.cleanup_sessions",
    max_retries=1,
    autoretry_for=(Exception,),
    retry_backoff=True,
    retry_jitter=True,
...
def cleanup_sessions(self, max_age_hours: int = 24) -> dict[str, Any]:
    ...
    sql_events = (
        "DELETE FROM session_events WHERE occurred_at < (NOW() - ($1 || ' hours')::interval)"
    )
    sql_envelopes = (
        "DELETE FROM session_envelopes WHERE updated_at < (NOW() - ($1 || ' hours')::interval)"
    )
...
@shared_task(
    bind=True,
    base=SafeTask,
    name="python.tasks.maintenance_tasks.dead_letter",
    max_retries=0,
    rate_limit="120/m",
)
def dead_letter(
