...
class RecallWait(Extension):
    async def execute(self, loop_data: LoopData = LoopData(), **kwargs):
        self.agent.context.log.log(type="debug", heading="RecallWait: execute called", content=f"iteration={loop_data.iteration}")
        set = settings.get_settings()
        task = self.agent.get_data(DATA_NAME_TASK_MEMORIES)
        iter = self.agent.get_data(DATA_NAME_ITER_MEMORIES) or 0
        self.agent.context.log.log(type="debug", heading="RecallWait: retrieved task", content=f"task={'exists' if task else 'None'}, iter={iter}")
        if task and not task.done():
            if set["memory_recall_delayed"]:
                if iter == loop_data.iteration:
                    ...
                    self.agent.context.log.log(type="debug", heading="RecallWait: delayed mode, inserted delay text")
                    return
