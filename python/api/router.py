...
class ChatRequest(BaseModel):
    ...
    agent_url: str = Field(
        ..., description="Remote FastA2A base URL", example="https://agent.example.com"
    )
...
async def chat_endpoint(
    request: ChatRequest, background: BackgroundTasks, timing: float = Depends(log_request_time)
):
    """
...
async def chat_status_endpoint(task_id: str, timing: float = Depends(log_request_time)):
    ...
    try:
        ...
        if task_result.get("started_at"):
            end_time = (
                task_result.get("completed_at") or task_result.get("failed_at") or time.time()
            )
        ...
        increment_counter(
            fast_a2a_requests_total,
            {"agent_url": "status_check", "method": "chat_status_endpoint", "status": "success"},
        )
...
async def chat_history_endpoint(
    session_id: str, limit: int = 50, timing: float = Depends(log_request_time)
):
    """
    ...
    try:
        ...
        increment_counter(
            fast_a2a_requests_total,
            {"agent_url": "history", "method": "chat_history_endpoint", "status": "success"},
        )
...
async def http_exception_handler(request, exc):
    ...
    return JSONResponse(
        status_code=exc.status_code, content={"detail": exc.detail, "timestamp": time.time()}
    )
...
async def general_exception_handler(request, exc):
    ...
    increment_counter(
        fast_a2a_errors_total,
        {"agent_url": "api", "error_type": type(exc).__name__, "method": "exception_handler"},
    )
