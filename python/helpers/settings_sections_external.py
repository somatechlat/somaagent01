...
def build_api_keys_section(settings: Any) -> SettingsSection:
    ...
    for p_type in ("chat", "embedding"):
        for provider in get_providers(p_type):
            ...
            fields.append({"id": f"api_key_{pid_lower}", "title": provider["label"],
                           "type": "password",
                           "value": API_KEY_PLACEHOLDER if has_secret else ""})

    return {"id": "api_keys", "title": "API Keys",
...
def build_secrets_section(settings: Any) -> SettingsSection:
    ...
    return {"id": "secrets", "title": "Secrets Management",
            "description": "Manage secrets and credentials.",
            "fields": [
                {"id": "variables", "title": "Variables Store",
                 "description": "Non-sensitive variables in .env format.",
                 "type": "textarea", "value": settings["variables"].strip(),
                 "style": "height: 20em"},
                {"id": "secrets", "title": "Secrets Store",
                 "description": "Secrets in .env format (masked).",
                 "type": "textarea", "value": secrets, "style": "height: 20em"},
            ], "tab": "external"}


def build_external_api_section() -> SettingsSection:
    """Build the external API section."""
    return {"id": "external_api", "title": "External API",
            "description": "Agent Zero external API endpoints.",
            "fields": [{"id": "external_api_examples", "title": "API Examples",
                        "type": "button", "value": "Show API Examples"}],
            "tab": "external"}
...
def build_litellm_section(settings: Any) -> SettingsSection:
    ...
    return {"id": "litellm", "title": "LiteLLM Global Settings",
            "description": "Global LiteLLM parameters.",
            "fields": [{"id": "litellm_global_kwargs", "title": "LiteLLM global parameters",
